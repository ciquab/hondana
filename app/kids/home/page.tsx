...
type SuggestionRow = { id: string; created_at: string; title: string | null; author: string | null; cover_url: string | null; status: string };
...
export default async function KidsHomePage({
  ...
  const [{ data: childRows }, { data: recentRows }, badges, { messages, unreadCount }, { data: suggestionRows }] = await Promise.all([
    ...
    <main className="relative mx-auto max-w-xl p-4 pb-28">
      ...
      <header className="mb-4 flex items-center justify-between rounded-2xl bg-amber-100 px-4 py-3">
        <div className="flex items-center gap-2">
          <span className="flex h-9 w-9 items-center justify-center rounded-full bg-white text-xl shadow-sm">🧒</span>
          <h1 className="text-xl font-bold text-amber-900">{child.display_name} のホーム</h1>
        </div>
        <form action={kidSignOut}>
          <button className="rounded-full border border-amber-300 bg-white px-3 py-1 text-sm text-amber-800">ログアウト</button>
        </form>
      </header>
      ...
      {suggestions.length > 0 && (
        <section className="mb-6 rounded-xl border border-orange-200 bg-orange-50 p-4">
          <p className="mb-2 text-xs font-semibold text-orange-700">📚 おとなからのおすすめ</p>
          <ul className="flex gap-3 overflow-x-auto pb-1">
            {suggestions.map((s) => {
              const title = s.title ?? '本';
              const { bg, text } = pickColor(s.id);
              return (
                <li key={s.id} className="flex-shrink-0">
                  {s.cover_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={s.cover_url} alt={title} className="h-20 w-14 rounded object-cover shadow" />
                  ) : (
                    <div
                    ...
                    >
                      <span
                        ...
                        style={{ writingMode: 'vertical-rl', color: text, maxHeight: '4.5rem', overflow: 'hidden' }}
                      >
                        {title.length > 14 ? `${title.slice(0, 14)}…` : title}
                      </span>
                    </div>
                  )}
                  <p className="mt-1 w-14 truncate text-center text-[10px] text-orange-800">{title}</p>
                </li>
              );
            })}
          </ul>
                      ...
                      (() => {
                        ...
                        return (
                          ...
                          >
                            <span
                              className="text-[10px] font-medium leading-tight"
                              style={{ writingMode: 'vertical-rl', color: text, maxHeight: '5.5rem', overflow: 'hidden' }}
                            >
