...
カラー（子どもモード寄り）:
- ベース: #FFFBF0（kidsレイアウト背景） or #FDE68A / #FDBA74 のやわらかい暖色
- 主アクセント: #F59E0B, #F97316, #FBBF24（amber/orange系）
- 補助アクセント: #0EA5E9 または #2563EB を少量（全体の15%以下）
- 高コントラストで視認性を優先（白モチーフ + 暖色背景を基本）
...
Color (kid-mode aligned):
- Base: #FFFBF0 or soft warm tones like #FDE68A / #FDBA74
- Primary accents: #F59E0B, #F97316, #FBBF24 (amber/orange family)
- Secondary accent: a small amount of #0EA5E9 or #2563EB (<=15% of total area)
- Prioritize strong contrast and legibility (white motif over warm base).
...
### この作例アイコンの講評（フィードバック用）
- **総評**: とても良いです。PWAアイコンとしては採用候補です。
- **良い点**:
  - 本 + 芽（ハート）のモチーフが一目で読書×成長を連想できる
  - 背景とモチーフのコントラストが十分で、小サイズでも認識しやすい
  - 余計なディテールが少なく、形状がシンプル
- **懸念点（軽微）**:
  - オレンジ + 緑の組み合わせは明るく親しみやすい一方、
    「安心・信頼」より「元気・活発」に寄る可能性がある
  - iOS/Androidの角丸マスクで外周の印象が少し変わるため、
    主モチーフを2〜4%だけ中央寄せ＆縮小するとより安全

### この作例をベースにした微調整プロンプト

```text
前回のアイコン構図（開いた本 + 芽/ハート）を維持し、視認性を保ったまま「安心感」を少し強めてください。

調整内容:
- 背景は kidsモードに合わせて「やわらかい amber/orange」へ統一（#F59E0B〜#F97316帯）
- 緑モチーフは彩度を少し下げ、必要なら amber 系ハイライトを追加して統一感を出す
- 本の白色は維持しつつ、ごく薄い陰影を追加して立体感を最小限だけ付与
- 主モチーフ全体を2〜4%縮小し、マスク切れ耐性を上げる
- 輪郭の太さは48px表示で潰れないよう維持

維持条件:
- 文字・ロゴなし
- 単一モチーフ中心
- 1024x1024 PNG（透明背景版も出力）
- 正方形原稿（角丸加工なし）
```

## 色調を「現在のアプリ内トーン」に合わせる指示テンプレート

現在のUIは `bg-slate-50`（背景）と `text-slate-900`（本文）を基調に、
ログイン主要CTAに `blue-600`、補助アクションに `slate-700` を使っています。
このため、画像生成時は「中明度の寒色グレー基調 + 青系アクセント少量」で指定するとズレにくいです。

### そのまま追記できる一文（ヒーロー画像向け）

```text
色調は、既存UIのTailwindトークンに厳密に合わせてください。
Base: slate-50 (#f8fafc), slate-900 (#0f172a)
Sub: white (#ffffff), slate-700 (#334155)
Accent: blue-600 (#2563eb) を少量、amber系は5%未満
全体は低彩度・中コントラストで、背景は明るく清潔感を保ってください。
```

### そのまま追記できる一文（PWAアイコン向け・子どもモード準拠）

```text
PWAアイコンは子どもモードに合わせた配色にしてください：
背景は #FFFBF0 を基調に、必要に応じて #FDE68A〜#FDBA74 の暖色グラデーションを使用。
主モチーフは #ffffff を中心に、アクセントは #F59E0B / #F97316 / #FBBF24。
青系（#0EA5E9 or #2563eb）は補助として面積15%以内に制限し、全体は明るく楽しい印象を優先してください。
```

### 生成後の色ズレ確認チェック
- 子どもモード画面（amber/orange基調）に置いた時に違和感がない
- 48pxでも白モチーフと背景の境界が明確に見える
- 暖色を増やしてもコントラスト不足になっていない
- iOS/Android角丸マスク時に主モチーフが欠けない
