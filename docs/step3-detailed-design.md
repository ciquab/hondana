...
- 子どもセッション: `child_session` クレームをJWTに付与し、`child_id` + `family_id` に限定（Phase D/E 準備を完了し、claim解釈ヘルパー・read/write RLS・DB role `child_session`・grants・`authenticator` からの引受許可を適用。アプリ kid導線の主要 read/write は child_session JWT + anon client 実行へ切替済み）
- 重要方針: 子どもは
  - 自分の記録作成/更新のみ可
...
現行実装では、子ども導線の更新系は **Server Actions** を中心に構成しています。
以下は「HTTP API 案」に対する実装対応です。

### 6.1 認証
- 実装: `verifyKidPin`（Server Action, `app/actions/kid-auth.ts`）
  - 入力: `childId`, `pin`
  - 出力: 署名付き `kid_session` Cookie をセットして `/kids/home` へ遷移
- `start` API は未分割（UIから直接 verify に送信）

### 6.2 記録補助
- 実装: `createKidRecord`（Server Action, `app/actions/kid-record.ts`）
  - 記録作成 + スタンプ保存 + 気持ちタグ保存を1アクションに集約

### 6.3 バッジ
- 実装: `evaluateChildBadges`（サーバー関数, `lib/kids/badges.ts`）
  - `createKidRecord` 完了時に呼び出し

### 6.4 カレンダー
- 実装: `/kids/calendar`（Server Component page）
  - `searchParams.month` を受け取り、月次集計を表示

### 6.5 招待
- 実装: `createInvite` / `acceptInvite` / `revokeInvite`（Server Actions, `app/actions/family.ts`）
- 画面: `/settings/family`（発行/無効化）・`/invite`（受け入れ）

### 6.6 メッセージ
- 実装: `/kids/messages`（Server Component page）+ `markKidMessageRead`（Server Action）
