...
- 子どもセッション: `child_session` クレームをJWTに付与し、`child_id` + `family_id` に限定（Phase D/E 準備として claim 解釈ヘルパーと read/write RLS スキャフォールドを追加済み。DB role `child_session` と grants も整備済み）
- 重要方針: 子どもは
  - 自分の記録作成/更新のみ可
