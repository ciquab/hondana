...
作成日: 2026-03-10  
更新日: 2026-03-10（完了状況反映 / Step 4前ミッション拡張追記）  
...
Step 3.9 は、既存機能の到達性を高める導線再編に加え、Step 4前にミッション運用の自由度を拡張するフェーズ。

- 親: 「今日やること」への到達を最短化
- 子: 「記録する」までの迷いを最小化
- 全体: 画面間でボタンの意味・強弱・配置ルールを統一
- ミッション: 子どもタイプ/家庭方針に応じて、目標設定の自由度を高める
...
- AI提案ロジック自体
- DBスキーマの大規模変更（最小限のカラム追加は 3.9-7 で許容）
- 通知基盤（配信キュー等）
...
Sprint D: 3.9-7 ミッション可変設定 → 3.9-8 親期待パターン別推薦
```
...
- ✅ Sprint D 初期実装完了（3.9-7/3.9-8）

---

## 3.9-7 ミッション自由度拡張（Step 4前倒し）

### 背景

- 既存ミッションはテンプレート選択 + 7日固定で、家庭ごとの目標設計幅が狭い。
- ペルソナレビューで提案された「波あり型」「自立重視」の家庭に対し、可変ミッションが有効。

### 方針

- 期間を `weekly`（7日）/`biweekly`（14日）で選択可能にする
- 目標値をテンプレ既定値 ±1 の範囲で調整可能にする
- 難易度プリセット（やさしい/ふつう/チャレンジ）から初期値を適用

### 実装メモ（最小差分）

- DB: `child_missions` に `duration_days` を追加（デフォルト 7）
- RPC: `set_child_mission` に `duration_days` / `custom_target_value` を追加
- UI: `MissionSetup` に期間・目標値の簡易入力を追加
- 計測: `mission_set` イベントに `duration_days`, `custom_target_value`, `preset` を含める

### 受け入れ条件

- [x] 親が 7日/14日を選んでミッション開始できる
- [x] 既存テンプレ運用（入力省略時）を壊さない
- [x] 目標値変更時も進捗判定が正しく動作する

---

## 3.9-8 親期待パターン別おすすめミッション

### 背景

- 親の期待（量/質/自立/関係性）に応じた提示がないため、テンプレ選択が迷いやすい。

### 方針

- 初回設定時に親の期待パターンを 1つ選択
- 選択結果に応じてテンプレート並び順・推奨文言・KPI表示を切替

### 受け入れ条件

- [x] 期待パターン選択時にテンプレ表示順が切り替わる
- [x] KPI表示（例: 量=頻度、質=完読率）がパターンに応じて変わる
- [x] Step 4 機能とは独立して動作する


### 進捗チェック（2026-03-10）

- 実装済み:
  - `app/actions/mission.ts`: `durationDays`/`customTargetValue` の受け渡し
  - `components/mission-setup.tsx`: 期間・目標値入力、親期待パターン選択、表示順最適化
  - `supabase/migrations/20260330000001_mission_sprint_d.sql`: `duration_days` / mission単位 `target_value` と RPC更新
- 次に進む項目（運用強化）:
  - 親期待パターンの永続化（初回設定）
  - 期待パターン別の重み最適化（計測データ反映）
