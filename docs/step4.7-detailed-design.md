...
- `/kids/*` での年齢モード判定と配信。
- UI コンポーネントのモード分岐（サイズ・文言・情報量）。
- AI 文言（4-1/4-2/4-3）で使う「文言モード」の共通I/F。
...
- [ ] 親設定の `age_mode_override` が `/kids/*` 表示に反映される
- [ ] `auto` の場合に生年から `junior/standard` が自動判定される
- [ ] `junior` で主要導線（home/new/complete）の視認性が改善される
- [ ] `standard` で既存操作性を劣化させない
- [ ] 導線イベントで `age_mode` を計測できる
- [ ] 4-1/4-2/4-3 側に `TextMode` 入力I/F が用意される
